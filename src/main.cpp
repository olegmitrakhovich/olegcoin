...
{
    string strBlockFn = strprintf("olg%04u.dat", nFile);
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    {
        try {
            CAutoFile olgdat(fileIn, SER_DISK, CLIENT_VERSION);
            unsigned int nPos = 0;
            while (nPos != (unsigned int)-1 && olgdat.good())
            {
                boost::this_thread::interruption_point();
                unsigned char pchData[65536];
                do {
                    fseek(olgdat, nPos, SEEK_SET);
                    int nRead = fread(pchData, 1, sizeof(pchData), olgdat);
                    if (nRead <= 8)
                    {
                        nPos = (unsigned int)-1;
                        break;
